                                    ...
                                    <table class="table-hover table-stripped table-bordered table">
                                        ...
                                        <tbody>
                                            <tr v-for="(item, loop) in requests?.data" :key="loop">
                                                ...
                                                <td>
                                                    <button v-if="item.status == 0" @click="loadItems(item.pid)" type="button" class="btn btn-primary btn-sm">
                                ...
                                <form id="itemForm" v-if="requestDetail?.items.length">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <textarea type="text" v-model="requestDetail.comment" class="form-control form-control-sm" placeholder="e.g UIU"></textarea>
...
function loadRequest() {
    store.commit('setSpinner', true)
    store.dispatch('getMethod', { url: '/load-raw-material-requests' }).then((data) => {
        store.commit('setSpinner', false)
        if (data.status == 200) {
