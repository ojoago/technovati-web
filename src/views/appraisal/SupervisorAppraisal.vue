<template>
    <div>
        <div class="container mt-2 mb-2">
            ...
            <div class="card mb-2" v-if="status">
                    ...
                    <form id="workForm">
                        ...
                        <fieldset class="border rounded-3 p-2 m-1">
                            <div class="row shadow m-2 rounded-3 px-3" v-for="(key, l) in object" :key="l">
                                ...
                                <div class="col-md-3">
                                    <RangeSlider v-model="key.supervisor_rating" min="0" :max="key.obtainable"
                                        :score="key.supervisor_rating" />
                                </div>
                            </div>
                        </fieldset>
                    </form>
                ...
                <div class="float-end p-4">
                    <textarea v-model="comment.supervisor_comment" cols="30" placeholder="Hr Comments" rows="5"
                        class="form-control form-controm-sm"></textarea>
                    <p class="text-danger" v-if="errors?.supervisor_comment">{{ errors?.supervisor_comment[0] }} </p>
        ...
        <o-modal :isOpen="summaryModal" modal-class="modal-sm" title="Assessment Summary" @modal-close="closeModal">
            <template #content>
                <div v-for="(key, loop) in summary?.supervisor_summary" :key="loop">
                    <p class="h5">{{ key?.key }}: {{ key?.score }}</p>
                    <hr>
                </div>
                <p class="h4"> Total: {{ summary?.supervisor_rating }} </p>
            </template>
            <template #footer>
                <div>
