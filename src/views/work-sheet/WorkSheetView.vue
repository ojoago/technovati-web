<template>
    <div>
        <div class="container mt-2">
            ...
            <div class="card">
                <div class="card-header h3">Work Sheet</div>
                <div class="card-body">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                     <select class="form-control form-control-sm" v-model="work.device"  >
                                                        ...
                                                        <option v-for="item in deviceDrop" :key="item.id" :value="item.id">{{ item.text }} </option>
                                                    </select>
                                                    <p class="text-danger " v-if="errors?.device">{{ errors?.device[0] }} </p>
                                                </div>
                                            </div>
                                            <div class="col-md-6">

                                                <div class="form-group">
                                                    <input type="date" v-model="work.date" class="form-control form-control-sm" placeholder="e.g 10">
                                                    <p class="text-danger " v-if="errors?.date">{{ errors?.date[0] }} </p>
                                                </div>
                                            </div>
                                           </div>                                            
                                            <div class="col-md-12" v-for="(member, loop) in members" :key="loop">
                                                <label class="form-label">{{ member.type == 2 ? member.casual.username : member.user.username }} {{ member.type == 2 ? member.casual.staff_id : member.user.staff_id  }} </label>
                                                <div class="input-group">
                                                    <input type="number" v-model="record[member.pid]" class="form-control form-control-sm" placeholder="e.g 10">
                                                </div>
                                            </div>
                                        </fieldset>
                                        <div class="float-end">
                                            <button type="button" class="btn btn-success btn-sm mt-2" @click="submitWorkSheet">Submit</button>
                                        </div>
                                    </form>
        ...
        <o-modal :isOpen="deviceModal" :modal-class="xs" title="Create Activity " @submit="createActivity" @modal-close="closeModal">
            <template #content>
                <form id="deviceForm">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" v-model="device.activity" class="form-control" placeholder="e.g activity A">
